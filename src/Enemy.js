...
export class Enemy extends Node {
  constructor(options = {}) {
    ...
    this.nextAttackTime = Math.random() * 10 + 10;

    this.laserSound = new Audio("../assets/sounds/laser.mp3");
    this.lightFlickerSound = new Audio("../assets/sounds/lightFlicker.mp3");
  }
  ...
  update(dt, player, time, light) {
    ...
    vec3.subtract(laserTranslation, laserTranslation, [
      -Math.sin(this.yaw),
      0,
      -Math.cos(this.yaw),
    ]);
  ...
  calculateAngleToPlayer(player) {
    return Math.atan2(
      player.translation[0] - this.translation[0],
      player.translation[2] - this.translation[2]
    );
  }
